...
module.exports = ({ sequelize, Sequelize }) => {
    ...
    return _deleteOldLocks()
      ...
      .catch(err => {
        if (attempt < maxAttempt) {
          logger.warning(`Cannot acquire the lock ${name}. Retry in ${delayInMs} milliseconds.`);
   ...
   * @return Promise
   */
  const _lock = (name, { attempt = 1, delay = 500, timeout = 60 }) =>
    _sync().then(() => _acquireLock(name, timeout, 1, attempt, delay));
